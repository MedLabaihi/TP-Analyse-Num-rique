...
disp(norm(A2,1)*norm(inv(A2),1),'Conditionement de A2 en norme 1')
disp(norm(A2,'inf')*norm(inv(A2),'inf'),'Conditionement de A2 en norme Inf')
disp(x2,'Solution de A2*x=b2 :')

disp(norm(inv(A1)*(A2-A1)),'||A1^(-1)(A2-A1)||')
disp(norm(x2-x1)/norm(x1),'||&x||/||x||')
